    ...
    >
      ...
      <a-form :form="form" @submit="handleSubmit">
        ...
        <a-form-item label="机构类型" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-select
            style="width:80%;"
            placeholder="请选择机构类型，可多选"
            v-decorator="['typeId',{rules: [{required: true, message: '请选择机构类型，可多选！'}]}]"
            mode="multiple"
            :allowClear="true"
          >
            <a-select-option
...
<script>
import { addMechanism, findAllMechanismType } from '@/api/manage'
      ...
      validateFields((errors, values) => {
        if (!errors) {
          ...
          this.fileListVideo.forEach(function (v) {
            values.advertVideo = v.response.data
          })
